    ...
    if dim == 1:
        ...
        for lv in range(1, level + 1):
            for i in range(0, nD):
                for j in range(0, nD):
                    Dec2[lv, i, j] = np.kron(
                        np.reshape(Dec[lv, i], (-1, 1)),
                        np.reshape(Dec[lv, j], (1, -1)),
                    )
        return Dec2
    elif dim == 3:
        Dec3 = {}
        for lv in range(1, level + 1):
            for i in range(0, nD):
                for j in range(0, nD):
                    for k in range(0, nD):
                        tmp = np.kron(
                            np.reshape(Dec[lv, i], (-1, 1, 1)),
                            np.reshape(Dec[lv, j], (1, -1, 1)),
                        )
                        Dec3[lv, i, j, k] = np.kron(
                            tmp, np.reshape(Dec[lv, k], (1, 1, -1))
                        )
        return Dec3
    ...
    if dim == 1:
        ...
        for lv in range(1, level + 1):
            for i in range(0, nD):
                for j in range(0, nD):
                    Rec2[lv, i, j] = np.kron(
                        np.reshape(Rec[lv, i], (1, -1)),
                        np.reshape(Rec[lv, j], (-1, 1)),
                    ).T
        return Rec2
    elif dim == 3:
        Rec3 = {}
        for lv in range(1, level + 1):
            for i in range(0, nD):
                for j in range(0, nD):
                    for k in range(0, nD):
                        tmp = np.kron(
                            np.reshape(Rec[lv, i], (-1, 1, 1)),
                            np.reshape(Rec[lv, j], (1, -1, 1)),
                        )
                        Rec3[lv, i, j, k] = np.kron(
                            tmp, np.reshape(Rec[lv, k], (1, 1, -1))
                        )
        return Rec3
