...
def splits(a: Tensor, sf: int):
    ...
    """
    sf_chunks = torch.chunk(a, sf, dim=2)
    dim_for_stack = 5
    if dim_for_stack > sf_chunks[0].ndim:
        dim_for_stack = sf_chunks[0].ndim
    b = torch.stack(sf_chunks, dim=dim_for_stack)
...
def p2o(psf, shape):
    ...
    otf[..., 1][torch.abs(otf[..., 1]) < n_ops * 2.22e-16] = torch.tensor(
        0
    ).type_as(psf)
    return otf
...
class USRNet(nn.Module):
    ...
    def forward(self, x: Tensor, k: Tensor, sf: int, sigma: Tensor):
        """
        x: tensor, NxCxWxH
        ...
        F2B = r2c(
            cabs2(FB)
        )  # квадрат модуля (или амплитуды) OTF (FB) и преобразуется обратно в комплексное представление.
